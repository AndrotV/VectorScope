...
function draw() {
    ...
    let x = AMP * Math.sin(t * 2*Math.PI * f1 + ph1); //map f1 to x
    let y = AMP * Math.sin(t * 2*Math.PI * f2 + ph2); //map f2 to y
    ...
    while (true) {
        ...
        for (t = t2; t <= t2 + 1/f1; t += 1/Math.max(f2,f1) / 100) {
            x = AMP * Math.sin(t * 2*Math.PI * f1 + ph1); //map f1 to x
            y = AMP * Math.sin(t * 2*Math.PI * f2 + ph2); //map f2 to y
